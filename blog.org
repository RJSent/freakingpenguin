...
First, a bus error can occur when we access a variable at an address
that's not valid for that variable. An address is not valid if the
address is not evenly divisible by the length of the variable. In
other words...
...
x86 is very forgiving when it comes to misalignment errors. This is
great for us, but what if we ported this code over to a platform that
is less friendly, like ARM?
...
By adding the ~-fsanitize=undefined~ compile flag, the code will print
a runtime error whenever one occurs.
...
at our code through a console, =stderr= and =stdout= might seem like
...
Unfortunately, these results don't include =stdout=. In order to see
...
Unfortunately, this is a limitation of =org-babel=. ~-fno-sanitize-recover=all~ causes
...
Given that this thread is 5 years old at this point, I'm not holding
my breath for a fix.
